...
public class COpenCutCroLookPicActivity extends DaggerAppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (CommonUtils.isNetAvailable()) {  //有网络情况下请求图片数据
            ...
            picsAdapter = new PicsAdapter(COpenCutCroLookPicActivity.this, false);
            binding.gv.setAdapter(picsAdapter);
            if (picList.size() != 0 && picList != null) {
                for (int i = 0; i < picList.size(); i++) {
                    String paths = picList.get(i);
                    cOpenCutCroViewModel.getPic(paths).observe(this, respEntityResource -> {
                        count++;
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if (CommonUtils.isNetAvailable()) {
            if (picdatas != null && picdatas.size() != 0) {
                //跳转到放大图
                new ImageDialog(COpenCutCroLookPicActivity.this, picdatas.get(position)).show();
            }
        } else {
//            无网络状态下
            if (picList.size() != 0 && picList != null) {
                new ImageDialog(COpenCutCroLookPicActivity.this, picList.get(position)).show();
            }
        }
    }
