 ...
 * Data:
 */
public interface COpenCutCroWebService {
    @Multipart
    @POST("/copencutcro/save")
    LiveData<Resource<RespEntity>> save(@PartMap Map<String, RequestBody> params, @Query("input_hidden_tablename") String tableName);


    @POST("/copencutcro/dataReport")
    LiveData<Resource<FlagRespEntity>> report(@Query("id") String id);

    @POST("/copencutcro/getDataListByStatus")
    LiveData<Resource<Response<List<COpenCutCroEntity>>>> list(@Query("page") int page, @Query("rows") int rows, @Query("tableId") String tableName, @Query("status") String status);

    @POST("/copencutcro/deleteById")
    LiveData<Resource<RespEntity>> delete(@Query("id") String id);

    @POST("/copencutcro/test")
    LiveData<Resource<RespEntity>> revoke(@Body List<COpenCutCroEntity> entities);

    @POST("/copencutcro/test")
    LiveData<Resource<RespEntity>> revoke(@Query("id") String id);

    @POST("/copencutcro/completeTaskJudge")
